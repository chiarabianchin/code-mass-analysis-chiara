...
fWeightForBkg(0),      
fDataPathFileForBkg(""),
fListInputDataBkgName(""),
fHistDataBkgName("fhnRhoVsRhoMVsLeadJetPtVsMassVsCent"),
fListInputDataBkg(0x0),
fNBinsPtForRho(1),
fLowLimsPtForRho(),
fRhoSelBin(),
fhSaveIntegralRhoBin(),
fRhoRespFactor(0)

{
   ...
   for(Int_t i = 0; i<fNpthB; i++ ){
		...
		fhMPt3DW   [i]   = 0x0;
		fhResponseN[i]   = 0x0;
		Printf("Here should be 0x0 -> pointer %p", fhResponseN[i]);
		fhptMpar   [i]   = 0x0;
		fhptMparW  [i]   = 0x0;
		fhptMdetW  [i]   = 0x0;
		fhptMparN  [i]   = 0x0;
   ...
   fhSaveIntegralRhoBin = new TH1D("fhSaveIntegralRhoBin", "Rho per pT hard bin", fNpthB, 0.5, fNpthB+0.5);
...
//// to be implemented!!!!
//}
void WeightClass::ClearMem(){
	
	Printf("Deleting histograms...");
	for(Int_t i = 0; i<fNpthB; i++ ){
		delete fhResponse [i] ;
		delete fhDMdPt    [i] ;
		delete fhMPt3D    [i] ;
		delete fhResponseW[i] ;
		delete fhDMdPtW   [i] ;
		delete fhMPt3DW   [i] ;
		Printf("Pointer %p", fhResponseN[i]);
		delete fhResponseN[i] ;
		delete fhptMpar   [i] ;
		delete fhptMparW  [i] ;
		delete fhptMdetW  [i] ;
		delete fhptMparN  [i] ;
		delete fhptMdetN  [i] ;
		delete fh2dResponse[i] ;
		delete fh2dResponseW[i] ;
	}
	...
	delete[] fhptMdetN     ;
	                       ;
	delete[] fh2dResponse  ;
	delete[] fh2dResponseW ;
	
	delete fhSaveIntegralRhoBin ;
...
void WeightClass::SetOrderAxesRespose(Int_t axMp, Int_t axMd, Int_t axptp, Int_t axptd){
	fAxisMd = axMd; fAxisMp = axMp; fAxisPtd = axptd; fAxisPtp = axptp;
	SetArrayOrderAxesResponse(); 
	Printf("Order of axes set to :  axMp = %d, axMd = %d, axptp = %d, axptd = %d", fAxisMp, fAxisMd, fAxisPtp, fAxisPtd);
	return;
}

//-------------------------------------------------------------------------------------------------

void WeightClass::SetOrderAxesMPt3D(Int_t axMp, Int_t axMd, Int_t axptp, Int_t axptd, Int_t axMdE, Int_t axptdE){
	fAxisMd = axMd; fAxisMp = axMp; fAxisPtd = axptd; fAxisPtp = axptp; fAxisMdEmb = axMdE; fAxisPtdEmb = axptdE; 
	Printf("SetAxesWithArrayMPt3D(). Axes set to :  axMp = %d, axMd = %d, axptp = %d, axptd = %d, axMdEmb = %d, axptdEmb = %d", fAxisMp, fAxisMd, fAxisPtp, fAxisPtd, fAxisMdEmb, fAxisPtdEmb);
	return;
}   

//-------------------------------------------------------------------------------------------------

void WeightClass::SetOrderAxesResposeWithRho(Int_t axMp, Int_t axMd, Int_t axptp, Int_t axptd, Int_t axr, Int_t axrm){
	if(!fWeightForBkg) {
		Printf("Error!! fWeightForBkg = kFALSE, exit with no settings!");
		
		return;
	}
	fAxisMd = axMd; fAxisMp = axMp; fAxisPtd = axptd; fAxisPtp = axptp; fAxisRho = axr; fAxisRhom = axrm; 
	SetArrayOrderAxesResponseWithRho(); 
	Printf("Order of axes set to :  axMp = %d, axMd = %d, axptp = %d, axptd = %d, axrho = %d, axrhom = %d", fAxisMp, fAxisMd, fAxisPtp, fAxisPtd, fAxisRho, fAxisRhom);
	return;
}
//-------------------------------------------------------------------------------------------------
...
void WeightClass::Initialise(TString path, TString list, TString inputFile, TString listnameInp){
   ...
   if(fAxisRho != -1 && fAxisRho != -1){
   	   Printf("include rho and rhom axes, fWeightForBkg is %d", fWeightForBkg);
   	   SetArrayOrderAxesResponseWithRho();
   }
   return;
...
Bool_t WeightClass::ReadRhoFromData(){
	
	// this method is needed to get the rho distribution from data output with its specific format and use it then for the normalization of the response
	// Read the output of AliAnalysisTaskJetMass (e.g. JetMass_Jet_AKTChargedR040_PicoTracks_pT0150_E_scheme_TCDeriv).
	// The histogram name is usually "fhnRhoVsRhoMVsLeadJetPtVsMassVsCent"
	if(!fWeightForBkg) 
	{
		Printf("No weighting requested");
		return kFALSE;
	}
	if(!fhResponseWFinal){
		Printf("The response is not yet weighted, using this method too early");
		return kFALSE;
	}
	TList *list = ReadFile(fDataPathFileForBkg, fListInputDataBkgName);
	if(!list){
		Printf("List %s not found in %s, not possible to weight for background", fListInputDataBkgName.Data(), fDataPathFileForBkg.Data());
		
		return kFALSE;
	}
	
	fhspMPtRhoData = (THnSparseF*) list->FindObject(fHistDataBkgName);
	if(!fhspMPtRhoData){
		Printf("%s not found in %s", fHistDataBkgName.Data(), fListInputDataBkgName.Data());
		fListInputDataBkg->ls();
		fWeightForBkg = kFALSE;
		return kFALSE;
	}
	
	
	Int_t rhoaxisdata = 0, rhomaxisdata = 1; 
	Int_t ptdaxisdata = 2;
	// Int_t maxisdata = 3, centaxisdata = 4;
	
	if(!fListInputDataBkg) {
		fListInputDataBkg = new TList();
		fListInputDataBkg->SetOwner();
	}
	for(Int_t iptbind = 0; iptbind < fNBinsPtForRho-1; iptbind++){
		
				
		// find the bins corresponding to the response pt range in the data thnsparse pt axis
		Double_t othertest = fLowLimsPtForRho[iptbind];
		Int_t test = fhspMPtRhoData->GetAxis(ptdaxisdata)->FindBin(fLowLimsPtForRho[iptbind]);
		Int_t binptd[2] =  {1, 2};//{fhspMPtRhoData->GetAxis(ptdaxisdata)->FindBin(fLowLimsPtForRho[iptbind]), fhspMPtRhoData->GetAxis(ptdaxisdata)->FindBin(fLowLimsPtForRho[iptbind+1]-0.001)};
		// make a selection on that bin range
		fhspMPtRhoData->GetAxis(ptdaxisdata)->SetRange(binptd[0], binptd[1]);
		
		// project the data thnsparse on the rho and rho_m axis 
		TH1D* hRho  = fhspMPtRhoData->Projection(rhoaxisdata);
		hRho  ->SetName(Form("hRhoDataPtD_%.0f_%.0f", fLowLimsPtForRho[iptbind], fLowLimsPtForRho[iptbind+1]));
		TH1D* hRhoM = fhspMPtRhoData->Projection(rhomaxisdata);
		hRhoM ->SetName(Form("hRhoMDataPtD_%.0f_%.0f", fLowLimsPtForRho[iptbind], fLowLimsPtForRho[iptbind+1]));
		
		fListInputDataBkg->Add(hRho);
		fListInputDataBkg->Add(hRhoM);
...
//-------------------------------------------------------------------------------------------------
Bool_t WeightClass::ReadRhoFromRespWF(){
// get the final response projection that I want to weight further
	
	if(!fhResponseWFinal) {
		Printf("Error! The weighted response final is not ready");
		return kFALSE;
	}
	if(!fListInputDataBkg){
		fListInputDataBkg = new TList();
	...
	Int_t ndim = fhResponseWFinal->GetNdimensions();
	if(ndim > fAxisRho && ndim > fAxisRhom){
		Printf("Info: projecting response final onto Rho (axis %d) and Rhom (axis %d) ", fAxisRho, fAxisRhom);
	} else {
		Printf("Axis indeces Rho (axis %d) and Rhom (axis %d) wrongly set or response matrix does not contain them (number of axes %d)", fAxisRho, fAxisRhom, ndim);
		return kFALSE;
	}

	// project the data thnsparse on the rho and rho_m axis 
	TH1D* hRho  = fhResponseWFinal->Projection(fAxisRho);
	hRho  ->SetName("hRhoResponse");
	TH1D* hRhoM = fhResponseWFinal->Projection(fAxisRhom);
	hRhoM ->SetName("hRhoMResponse");
...
//-------------------------------------------------------------------------------------------------
Bool_t WeightClass::RhoFactors(){
	// ratios of the data/response rho distributions
	if(!fListInputDataBkg){
		Printf("List of rho distributions not ready, run ReadRhoFromData() and ReadRhoFromRespWF() first");
		return kFALSE;
	}
	for(Int_t iptbind = 0; iptbind < fNBinsPtForRho-1; iptbind++){
		...
		TH1* hRatioRho = 0x0;
		TH1* h = 0x0;
		TH1D* hRhoData = (TH1D*)fListInputDataBkg->FindObject(Form("hRhoDataPtD_%.0f_%.0f", fLowLimsPtForRho[iptbind], fLowLimsPtForRho[iptbind+1]))->Clone(Form("hRhoRatioPtD_%.0f_%.0f", fLowLimsPtForRho[iptbind], fLowLimsPtForRho[iptbind+1]));
		

		TH1D* hRhoResp =  (TH1D*)fListInputDataBkg->FindObject("hRhoResponse");
		
		if(!hRhoData || !hRhoResp){
			Printf("Cannot make ratio, bin %d", iptbind);
			continue;
		}
		UniformTH1FForDivide(hRhoData, hRhoResp, hRatioRho, h, "TH1D");
		
		hRatioRho->Divide(hRhoResp);
		
		fListInputDataBkg->Add(hRatioRho);
	}
...
void WeightClass::ReadInputData(Int_t ipthb, THnSparseF* hspresp){
   ...
   TString suffix = "";
   if(fRhoSelBin[0] != -1 && fRhoSelBin[1] != -1) {
   	   Printf("Rho selection %f %f", fRhoSelBin[0], fRhoSelBin[1]);
   	   TAxis *ax = hspresp->GetAxis(6);
   	   if(!ax) Printf("Can't proceed with the rho selection, axis 6 not found, max %d", hspresp->GetNdimensions());
   	   else {
   	   	   TH1D* hRho = hspresp->Projection(6);
   	   	   //Double_t total = hRho->Integral();
   	   	   Int_t binrho[2] = {hspresp->GetAxis(6)->FindBin(fRhoSelBin[0]), hspresp->GetAxis(6)->FindBin(fRhoSelBin[1] - 0.001)};
   	   	   //make projection on rho axis
   	   	   hspresp->GetAxis(6)->SetRange(binrho[0], binrho[1]);
   	   	   //hRho = hspresp->Projection(6);
   	   	   //Printf("0 or 1? %d", ipthb);
   	   	   //fhSaveIntegralRhoBin->SetBinContent(ipthb + 1, hRho->Integral()/total);
   	   	   //Printf("Filled %d with %f", ipthb, hspresp->Projection(6)->Integral());
   	   	   
   	   	   //hspresp->GetAxis(6)->SetRange(0, -1);
   	   	   suffix = Form("Rho%.0f_%.0f", fRhoSelBin[0]*10, fRhoSelBin[1]*10);
   	   }
   }
   if(!fWeightForBkg) fhResponse[ipthb] = (THnSparseF*)hspresp->ProjectionND(fnD, fAxesResp, "E");
   else fhResponse[ipthb] = (THnSparseF*)hspresp->ProjectionND(fnD, fAxesRespWithRho, "E");
   fhResponse[ipthb]->SetName(Form("hResponse%s_pTh%d%s", GetName(), ipthb+1, suffix.Data()));
   fhResponse[ipthb]->Sumw2();

   for(Int_t iax = 0; iax < fnD; iax++){
   ...
   if(fAxisRho != -1 && fAxisRho != -1){
   	   //after the ND projection, this is the order
   	   fAxisMd = 0; fAxisMp = 1; fAxisPtd = 2; fAxisPtp = 3; fAxisRho = 4; fAxisRhom = 5;
   	   SetArrayOrderAxesResponseWithRho();
   }
...
//-------------------------------------------------------------------------------------------------
Bool_t WeightClass::CleanOutliers(Int_t ipthb){
	
	if(!fhResponse[ipthb]){
		Printf("Error, no response set yet");
		return kFALSE;
	} 
	if(fnD != 4) {
	...
	Int_t counter = 0;
	for(Int_t i = 0; i < fhResponse[ipthb]->GetAxis(0)->GetNbins(); i++) {
		
		for(Int_t j = 0; j < fhResponse[ipthb]->GetAxis(1)->GetNbins(); j++){
		
			for(Int_t k = 0; k < fhResponse[ipthb]->GetAxis(2)->GetNbins(); k++){
			
				for(Int_t l = 0; l < fhResponse[ipthb]->GetAxis(3)->GetNbins(); l++){
					
					Int_t bin[4] = {i, j, k, l};
					Float_t content = fhResponse[ipthb]->GetBinContent(bin);
					if(content > 0 && content < fMinAcceptableCont){
						fhResponse[ipthb]->SetBinContent(bin, 0);  
						counter++;
					}
				}
			}
	...
	Printf("Set %d bin to zero", counter);
	return kTRUE;
}
...
Bool_t WeightClass::WeightResponse(Int_t ipthb){
   ...
   TString suffix = "";
   if(fRhoSelBin[0] != -1 && fRhoSelBin[1] != -1) Form("Rho%.0f_%.0f", fRhoSelBin[0]*10, fRhoSelBin[1]*10);
   
   fhResponseW[ipthb] = (THnSparseF*)fhResponse[ipthb]->Clone(Form("fhResponseW_pTh%d%s", ipthb+1, suffix.Data()));
   ...
   //}
      ...
      Printf("No normalization");
      fhResponseN[ipthb] = 0x0;
...
void WeightClass::NormalizePerIntegral(Int_t ipthb){
   ...
   Int_t rebinall[fnD];
   for(Int_t iax = 0; iax < fnD; iax++) rebinall[iax] = 2; //to be improved!!! //{4,4,10,10};
   THnSparseF* fhResponseWRebin = (THnSparseF*)fhResponseW[ipthb]->Rebin(rebinall); 
   
   TString suffix = "";
   if(fRhoSelBin[0] != -1 && fRhoSelBin[1] != -1) Form("Rho%.0f_%.0f", fRhoSelBin[0]*10, fRhoSelBin[1]*10);
   
   fhResponseN[ipthb] = (THnSparseF*)fhResponseWRebin->Clone(Form("fhResponseWNormPerJet_pthb%d%s", ipthb, suffix.Data()));
...
//-------------------------------------------------------------------------------------------------
Int_t WeightClass::CheckBinning(Int_t ipthb, TH1D* h, Int_t ax, TString level) const {
   /// Compare the particle level binning and range of the current response (pT hard bin ipthb or the weighted final if ipthb is negative) with that of the input 1D histograms
   /// It returns the rebinning factor to be applied to the response to match the binning of the input
   /// level: determines with which response to compare (maybe to be improved), see below
   
   THnSparseF *hcurrent = 0x0;
   
   if(ipthb > fFirstpthB-1 && ipthb < fNpthB) {
   	   if(level == "W") hcurrent = fhResponseW[ipthb];
   	   if(level == "N") hcurrent = fhResponseN[ipthb];
   }
   if(ipthb < 0) hcurrent = fhResponseWFinal;
   
   Double_t rangePPar[2] = {hcurrent->GetAxis(ax)->GetBinLowEdge(1), hcurrent->GetAxis(ax)->GetBinLowEdge(hcurrent->GetAxis(ax)->GetNbins() + 1)};
   Double_t binWidthPPar = hcurrent->GetAxis(ax)->GetBinWidth(5);
...
void WeightClass::NormalizePerBin(Int_t ipthb){
   ...
   Int_t rebPPt = CheckBinning(ipthb, hNJetsPerPtPBinThisPtHBin, fAxisPtp); 
   ...
   rebinall[3] = factorPt*rebPPt; //{2,2,2,2}; //{4,4,10,10};
   for(Int_t iax = 4; iax < fnD; iax++) rebinall[iax] = 5;
   ...
   Printf("In normalise the pointer %d is %p", ipthb, fhResponseN[ipthb]);
   ...
   Int_t nBinsRho = 1;
   Int_t nBinsRhom = 1;
   Printf("ax rho = %d/%d", fAxisRho, fhResponseN[ipthb]->GetNdimensions());
   if(fWeightForBkg) {
   	   nBinsRho = fhResponseN[ipthb]->GetAxis(fAxisRho)->GetNbins();
   	   nBinsRhom = fhResponseN[ipthb]->GetAxis(fAxisRhom)->GetNbins();
   }
   
   fhResponseN[ipthb]->Reset();
   //Printf("Dimensions %d, proj %d, %d", fhResponseWRebin->GetNdimensions(), fAxisMp, fAxisPtp);
   Printf("Starting a loop over %d Pt bins, %d Mass bins at particle level \nPer each of these bins a loop over %d Pt bins and %d M bins at detector level, %d Rho, %d Rhom will be run. Might take a while", nBinsPPar, nBinsMPar, nBinsPDet, nBinsMDet, nBinsRho, nBinsRhom);
      ...
      for(Int_t ibmpar = 0; ibmpar < nBinsMPar; ibmpar++){ //M par
      	  ...
      	  for(Int_t ibpdet = 0; ibpdet < nBinsPDet; ibpdet++){ // Pt det
      	  	  ...
      	  	  for(Int_t ibmdet = 0; ibmdet < nBinsMDet; ibmdet++){ // M det
      	  	  	  ...
      	  	  	  for(Int_t ibrho = 0; ibrho < nBinsRho; ibrho++){ // Rho
      	  	  	  	  for(Int_t ibrhom = 0; ibrhom < nBinsRhom; ibrhom++){ // Rhom
      	  	  	  	  	  Int_t thisbin6[6] = {ibmdet+1, ibmpar+1, ibpdet+1, binRespW, ibrho+1, ibrhom+1}; //same order as the original response
      	  	  	  	  	  Int_t thisbin4[4] = {ibmdet+1, ibmpar+1, ibpdet+1, binRespW};
      	  	  	  	  	  
      	  	  	  	  	  Int_t *thisbin = 0x0;
      	  	  	  	  	  if(fnD == 4) thisbin = thisbin4;
      	  	  	  	  	  if(fnD == 6) thisbin = thisbin6;
      	  	  	  	  	  // debugging
      	  	  	  	  	  //Printf("this bin pointer %p, 4 - %p, 6 - %p", thisbin, thisbin4, thisbin6);
...
//-------------------------------------------------------------------------------------------------
void WeightClass::WeightForRho(){
	
	if(!fListInputDataBkg){
		Printf("WeightForRho::Rho input from data and resp missing");
		return;
	}
	if(!fhResponseWFinal){
		Printf("WeightForRho::Response weighted final missing");
		return;
	}
	fhResponseWFinalRho = (THnSparseF*)fhResponseWFinal->Clone(Form("fhResponseWFinalRho"));
	fhResponseWFinalRho->Reset();
	
	TH1D *hcurrentRho = 0x0;
	
	//nbins of all reponse axes
	Int_t nBinsPDet = fhResponseWFinal->GetAxis(fAxisPtd)->GetNbins(); 
	Int_t nBinsPPar = fhResponseWFinal->GetAxis(fAxisPtp)->GetNbins();
	Int_t nBinsMPar = fhResponseWFinal->GetAxis(fAxisMp)->GetNbins();
	Int_t nBinsMDet = fhResponseWFinal->GetAxis(fAxisMd)->GetNbins();
	Int_t nBinsRho  = fhResponseWFinal->GetAxis(fAxisRho)->GetNbins();
	Int_t nBinsRhom = fhResponseWFinal->GetAxis(fAxisRhom)->GetNbins();
	...
	for(Int_t ibpdet = 0; ibpdet < nBinsPDet; ibpdet++){
		...
		// define what is the rho ratio to be used:
		Double_t ptdetvalue = fhResponseWFinal->GetAxis(fAxisPtd)->GetBinCenter(ibpdet+1);
		Int_t ptdetbin = 0; 
		
		for(ptdetbin = 0; ptdetbin<fNBinsPtForRho-1; ptdetbin++ ){
			// if the value is in the range checked, just exit, the bin number is saved in ptdetbin
			if(ptdetvalue >= fLowLimsPtForRho[ptdetbin] && ptdetvalue < fLowLimsPtForRho[ptdetbin+1]) break;
      
		}
		
		Printf("The pt bin used is %d", ptdetbin);
		if(!hcurrentRho || previous != ptdetbin){
			Printf("Reading hRhoRatioPtD_%.0f_%.0f", fLowLimsPtForRho[ptdetbin], fLowLimsPtForRho[ptdetbin+1]);
			hcurrentRho = (TH1D*)fListInputDataBkg->FindObject(Form("hRhoRatioPtD_%.0f_%.0f", fLowLimsPtForRho[ptdetbin], fLowLimsPtForRho[ptdetbin+1]));
		}
		previous = ptdetbin;
		...
		for(Int_t ibmpar = 0; ibmpar < nBinsMPar; ibmpar++){ //M par
			...
			for(Int_t ibmdet = 0; ibmdet < nBinsMDet; ibmdet++){ // M det
				for(Int_t ibptpar = 0; ibptpar < nBinsPPar; ibptpar++){ //Pt par
					//loop on pTdet, Mdet
				for(Int_t ibpdet = 0; ibpdet < nBinsPDet; ibpdet++){
				
					for(Int_t ibrho = 0; ibrho < nBinsRho; ibrho++){ // Rho
						for(Int_t ibrhom = 0; ibrhom < nBinsRhom; ibrhom++){ // Rhom
							Int_t thisbin6[6] = {ibmdet+1, ibmpar+1, ibpdet+1, ibpdet+1, ibrho+1, ibrhom+1}; //same order as the original response
							//
							//use the content of each bin to fill the normalized ThnSparse per each pTdet, Mdet and pTpar,Mpar
							
							Double_t factor = hcurrentRho->GetBinContent(ibrho); 
							if(factor - 1 < 1e-13) {
								factor = 1;
							}
							...
							fhResponseWFinalRho->SetBinContent(thisbin6, fhResponseWFinal->GetBinContent(thisbin6) * factor);
							fhResponseWFinalRho->SetBinError(thisbin6,  fhResponseWFinal->GetBinError(thisbin6) * factor);
...
Bool_t WeightClass::Result(){
   ...
   for(Int_t ipthb = fFirstpthB; ipthb < fNpthB; ipthb++){
   	  ...
   	  //fRhoRespFactor += fhSaveIntegralRhoBin->GetBinContent(ipthb+1);
   	  //Printf("Updated factor = %f", fRhoRespFactor);
      if(fIs2D) {
      	 ...
      	 if(!fhResponseW[ipthb]) {
      	    ...
      	    if(fDoNormPerInt){ // in this case I have the response in fhResponseN
      	       ...
      	       if(!fhResponseN[ipthb]){
      	       	   Printf("Weighted twice response %d not found, return", ipthb);
      	       	  return 0;
      	       }
      	    } else { // fhResponseN will be used in the sum, so I put the content of fhResponseW in it
      	       fhResponseN[ipthb] = (THnSparseF*)fhResponseW[ipthb]->Clone(Form("%sCp", fhResponseW[ipthb]->GetName()));
      	       Printf("not normalised, Normalized %p, original %p", fhResponseN[ipthb], fhResponseW[ipthb]);
      	    }
      	 }
      	 if(!fhResponseWFinal) {
      	 	 Printf("Pointer pthbin %d %p", ipthb, fhResponseN[ipthb]);
