...
EMBTYPEDIR=SingleMassiveDistribTrackInThermal10k
...
MASS=0         # single track is massless = -1
PATHFILEMASS=/data/Work/jets/JetMass/DetectorCorrections/LHC13b4_plus/Train576/MassEmbInput_fhnMassResponse_proj1_Sel3-40120.root # se to null if don't want to use it
NAMEMASSH=fhnMassResponse_proj_1 # se to null if don't want to use it
...
echo "PATHFILEMASS=$PATHFILEMASS" >>$FILESETTINGS
echo "NAMEMASSH=$NAMEMASSH" >>$FILESETTINGS
if [ "$SIMU" -eq 1 ]; then
#run simulation
...
do
   ...
   if [ $SINGLEPART -eq 1 ]; then
      root -b -q "$BASEDIR/$SUBDIR/runTestJetMassDerivWithScript.C($bininput, $PYTHIAJET, $THERMAL, $EMBCODE, $SINGLEPART, $JETRECSCHEME, $FRACSHARE, $PTHARDMin, $PTHARDMax, $PTPARTMin, $PTPARTMax, \"ESD\", \"local\", $MASS, \"$PATHFILEMASS\", \"$NAMEMASSH\")"
