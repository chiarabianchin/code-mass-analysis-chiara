...
MINPTPCUT=10.
MaxEntries=1100000
BASEOUTNAME=outRter1
for j in `seq 1 9`;
do
    cd $BASEDIR/0$j
    pwd
    root -b -q "/data/Work/MyCodeJetMass/macros/RandomizeTreeForEmbedding.C+(\"AnalysisResults.root\", \"fTreeJe", 0, $j, $MINPTDCUT, $MINPTPCUT, $MaxEntries, \"$BASEOUTNAME\" )"
    ls
